...
class CounterControl extends LitElement {
  ...
  render() {
    return html`
      <span>
        <fws-button @click="${this.handleDecrease}">-</fws-button>
        <input data-value type="number" min="0" value="${this.value}">
        <fws-button @click="${this.handleIncrease}">+</fws-button>
      </span>
      <fws-button @click="${this.handleReset}">reset</fws-button>
    `
  }
}
